...
func NewUsageWindowStatsCalculator(ctx context.Context, db *gorm.DB, costResolvers ...pricing.Resolver) (*UsageWindowStatsCalculator, error) {
	if db == nil {
	...
	if len(costResolvers) == 1 {
		...
		snapshot, err := LoadPricingSnapshot(ctx, db)
		if err != nil {
			return nil, err
		}
