...
func OpenDatabasePools(cfg config.Config) (*gorm.DB, *gorm.DB, error) {
	...
	resolver := dbresolver.Register(dbresolver.Config{
		Replicas: []gorm.Dialector{&sqlite.Dialector{Conn: readerSQL}},
	})
...
func sqliteReadDSN(path string) (string, error) {
	...
	// 文件名和 query 参数分开处理，避免简单追加产生重复 mode 或 query_only PRAGMA。
	trimmed := strings.TrimSpace(path)
	...
	query.Set("mode", "ro")
	setSQLitePragma(query, "query_only", "1")
	query.Set("cache", "private")

	// SQLite 只有 file: URI 才会把 mode=ro 作为 URI 参数处理；绝对化还能避免相对路径被误作 URI authority。
	...
	// 返回唯一 query string；SQLite core 处理 mode=ro，pure-Go driver 逐条应用 _pragma 参数。
	return uriFilename + "?" + query.Encode(), nil
}

// setSQLitePragma 删除同名 PRAGMA 的调用方值后追加唯一强制值，其它自定义 PRAGMA 保持不变。
func setSQLitePragma(query url.Values, name, value string) {
	normalizedName := strings.ToLower(strings.TrimSpace(name))
	pragmas := query["_pragma"]
	filtered := make([]string, 0, len(pragmas)+1)
	for _, pragma := range pragmas {
		normalizedPragma := strings.ToLower(strings.TrimSpace(pragma))
		if separator := strings.IndexAny(normalizedPragma, "=("); separator >= 0 {
			normalizedPragma = strings.TrimSpace(normalizedPragma[:separator])
		}
		if normalizedPragma != normalizedName {
			filtered = append(filtered, pragma)
		}
	}
	query["_pragma"] = append(filtered, name+"("+value+")")
}
