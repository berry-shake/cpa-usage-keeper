...
func TestOpenDatabaseRejectsFileWhenVFSDoesNotEnableWAL(t *testing.T) {
	...
	dsn := "file:" + filepath.ToSlash(dbPath) + "?vfs=unix-none&_pragma=busy_timeout(5000)&_pragma=foreign_keys(1)"
...
func TestOpenDatabaseClosesPoolWhenMigrationInitializationFails(t *testing.T) {
	...
	dsn := dbPath + "?_pragma=locking_mode(EXCLUSIVE)&_pragma=busy_timeout(50)&_pragma=foreign_keys(1)"
	db, err := repository.OpenDatabase(config.Config{SQLitePath: dsn})
	if db != nil {
		closeTestDatabase(t, db)
	}
	...
	verificationDB, err := sql.Open("sqlite", dbPath+"?_pragma=busy_timeout(50)")
	if err != nil {
...
func TestOpenReadDatabaseConfiguresBoundedReadOnlyPool(t *testing.T) {
	...
	// 断言：query_only 必须通过 DSN 的 _pragma 应用到每条物理连接，而不是只配置第一条连接。
...
func TestOpenReadDatabaseOverridesConflictingDSNProtection(t *testing.T) {
	...
	closeTestDatabase(t, writer)
	readerPath := dbPath + "?mode=rwc&_pragma=query_only(0)&_pragma=busy_timeout(2500)&_pragma=foreign_keys(1)"
	...
	// 断言：冲突的 query_only(0) PRAGMA 必须被唯一的强制值覆盖。
	var queryOnly int
	if err := connection.QueryRowContext(context.Background(), "PRAGMA query_only").Scan(&queryOnly); err != nil {
