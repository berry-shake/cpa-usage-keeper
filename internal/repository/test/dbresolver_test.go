...
func TestOpenDatabaseUsesPinnedSQLiteVersion(t *testing.T) {
	// 准备：通过项目正式数据库入口打开全新文件，版本必须来自 fork 固定的 pure-Go SQLite 依赖。
	...
	// 断言：glebarez/go-sqlite v1.21.2 固定内嵌 modernc SQLite 3.41.2，防止依赖解析偏离 pure-Go 基线。
	if version != "3.41.2" {
		t.Fatalf("expected SQLite 3.41.2 from glebarez/go-sqlite v1.21.2, got %q", version)
	}
}
