...
func TestRemoveUsageEventWriteHeavyIndexesMigrationDropsInsertCostIndexes(t *testing.T) {
	...
	for _, statement := range []string{
		...
		`CREATE INDEX idx_usage_events_api_group_key_timestamp_id ON usage_events(api_group_key, timestamp DESC, id DESC)`,
		`CREATE INDEX idx_usage_events_event_key ON usage_events(event_key)`,
		`CREATE INDEX idx_usage_events_failed ON usage_events(failed)`,
	} {
	...
	if err := removeUsageEventWriteHeavyIndexesMigration(db); err != nil {
		t.Fatalf("removeUsageEventWriteHeavyIndexesMigration returned error: %v", err)
	}
	...
	for _, indexName := range []string{
		...
		if sqliteIndexExists(t, db, indexName) {
			t.Fatalf("expected write-heavy index %s to be dropped", indexName)
		}
	}
	for _, indexName := range []string{
